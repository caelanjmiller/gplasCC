...
def check_prediction(sample, path_prediction, outdir):
    ...
    if prediction_file.shape[1] != 5:
        raise PredictionFileFormatError(
            "Prediction file should contain 5 tab-separated columns"
        )
    ...
    if prediction_file.columns[0] != "Prob_Chromosome":
        raise PredictionFileFormatError(
            "First column should be named Prob_Chromosome (case sensitive)"
        )

    if prediction_file.columns[1] != "Prob_Plasmid":
        raise PredictionFileFormatError(
            "Second column should be named Prob_Plasmid (case sensitive)"
        )

    if prediction_file.columns[2] != "Prediction":
        raise PredictionFileFormatError(
            "Third column should be named Prediction (case sensitive)"
        )

    if prediction_file.columns[3] != "Contig_name":
        raise PredictionFileFormatError(
            "Fourth column should be named Contig_name (case sensitive)"
        )

    if prediction_file.columns[4] != "Contig_length":
        raise PredictionFileFormatError(
            "Fifth column should be named Contig_length (case sensitive)"
        )
    ...
    if not is_float_dtype(
        prediction_file.dtypes["Prob_Chromosome"]
    ) and not is_integer_dtype(prediction_file.dtypes["Prob_Chromosome"]):
        raise PredictionFileFormatError("First column should be of type float/integer")

    if (
        max(prediction_file["Prob_Chromosome"]) > 1
        or min(prediction_file["Prob_Chromosome"]) < 0
    ):
        raise PredictionFileFormatError(
            "First column should contain values between 0 and 1"
        )

    if not is_float_dtype(
        prediction_file.dtypes["Prob_Plasmid"]
    ) and not is_integer_dtype(prediction_file.dtypes["Prob_Plasmid"]):
        raise PredictionFileFormatError("Second column should be of type float/integer")

    if (
        max(prediction_file["Prob_Plasmid"]) > 1
        or min(prediction_file["Prob_Plasmid"]) < 0
    ):
        raise PredictionFileFormatError(
            "Second column should contain values between 0 and 1"
        )

    if not is_object_dtype(
        prediction_file.dtypes["Prediction"]
    ) and not is_string_dtype(prediction_file.dtypes["Prediction"]):
        raise PredictionFileFormatError(
            "Third column should be of type character/string"
        )

    valid_predictions = ["Plasmid", "Chromosome"]
    if any([pred not in valid_predictions for pred in prediction_file["Prediction"]]):
        raise PredictionFileFormatError(
            f"Third column values should be either {' or '.join(valid_predictions)} (case sensitive)"
        )

    if not is_object_dtype(
        prediction_file.dtypes["Contig_name"]
    ) and not is_string_dtype(prediction_file.dtypes["Contig_name"]):
        raise PredictionFileFormatError(
            "Fourth column should be of type character/string"
        )
    ...
    if "Plasmid" not in prediction_file["Prediction"].values:
        raise PredictionFileFormatError(
            "There are no plasmids in the prediction file, gplas can't do anything"
        )
    ...
    if not all(comparison_output):
        raise PredictionFileFormatError(
            f"Contig names in prediction file should match exactly with those in '{raw_nodes_path}'"
        )
