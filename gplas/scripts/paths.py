...
def generate_paths(
    sample, number_iterations, filtering_threshold, outdir, sd_coverage=1, mode="normal"
):
    # Inputs
    ...
    def plasmid_graph(
        ...
        for iteration in range(
            ...
            info_path = info_path[
                [number not in repeats_signed_nodes for number in info_path["number"]]
            ]  # Removing the contigs corresponding to transposases
            ...
            for elongation in range(number_nodes):
                ...
                up_cutoff = (
                    cov_connections_info.loc[:, "coverage"].copy() + max_variation
                )
                down_cutoff = (
                    cov_connections_info.loc[:, "coverage"].copy() - max_variation
                )

                up_threshold = scipy.stats.norm.cdf(
                    up_cutoff, loc=path_mean, scale=max_variation
                )
                down_threshold = scipy.stats.norm.cdf(
                    down_cutoff, loc=path_mean, scale=max_variation
                )
                ...
                if (
                    len(path) >= number_nodes
                ):  # We only exit the function if we have reached the maximum number of nodes allowed per path
                    output = "\t".join(path)
                    paths_list.append(output)
                    path = [initial_seed]
                    break  # Exiting elongation loop
