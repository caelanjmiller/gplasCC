...
def coverage(sample, path_prediction, pred_threshold, outdir):
    ...
    if kc_check == len(raw_contig_names):
        ...
        coverage = [
            coverage / statistics.median(kc_coverage) for coverage in kc_coverage
        ]
    else:
    ...
    graph_pos_contigs = pd.DataFrame(
        data={
    ...
    graph_neg_contigs = pd.DataFrame(
        data={
            "number": [digit + "-" for digit in number],
            "length": lengths,
            "coverage": coverage,
    ...
    repeats = repeat_info[
        (repeat_info["indegree"] > 1) | (repeat_info["outdegree"] > 1)
    ]
    ...
    final_prediction = pred[
        [number not in list(repeats["number"]) for number in pred["number"]]
    ]
    ...
    isolated_nodes = contig_info[
        [number not in unique_nodes_signless for number in contig_info["number"]]
    ]
    ...
    repeats_final = pred[
        [number in list(repeats["number"]) for number in pred["number"]]
    ]
    ...
    pl_nodes = pl_nodes[
        pl_nodes["Contig_length"] > 500
    ]  # TODO ASK Julian: is this threshold needed?
    ...
    chr_contigs = pred[
        (pred["Prob_Chromosome"] > 0.7) & (pred["Contig_length"] > 1000)
    ]  # TODO this is hardcoded, both values could/should become parameters?
    cov_estimation = chr_contigs[
        [number not in list(repeats["number"]) for number in chr_contigs["number"]]
    ]
