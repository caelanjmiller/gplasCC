...
def partitioning_components(graph):
    ...
    partition_info = pd.DataFrame(
        data={
            "Algorithm": ["Walktrap", "Leading-eigen", "Louvain"],
...
def calculate_coocurrence(
    ...
    clean_pred = clean_pred.astype(
        {
            "Prob_Chromosome": float,
            "Prob_Plasmid": float,
            "Prediction": str,
            "Contig_name": str,
            "Contig_length": int,
    ...
    solutions = pd.read_csv(
        input_solutions, sep="\t", header=None, names=range(max_nodes), engine="python"
    )
    ...
    starting_nodes = [
        node for node in unique_nodes if node in solutions.loc[:, 0].values
    ]
    ...
    for node in starting_nodes:
        index_col = [
            col_name == node for col_name in unique_nodes
        ]  # select the column of the target node
        index_walks = co_ocurrence.loc[
            :, index_col
        ].values  # select all walks where the target node is present
        walks = co_ocurrence.loc[index_walks, :].copy()
        col_sums = [
            sum(walks.loc[:, col]) for col in walks
        ]  # count how often any node is present in all selected walks
        for index_connecting_node in range(
            len(col_sums)
        ):  # save coocurrence data for every node/connecting_node
    ...
    if len(circular_sequences) > 0:
        no_duplicated = [
            sorted(list(unique_walk))
            for unique_walk in set(tuple(walk) for walk in circular_sequences)
    ...
    total_pairs = pd.DataFrame(
        total_pairs, columns=["Starting_node", "Connecting_node", "weight"]
    )

    total_pairs.loc[:, "Starting_node"] = [
        node.replace("+", "") for node in total_pairs.loc[:, "Starting_node"]
    ]
    total_pairs.loc[:, "Starting_node"] = [
        node.replace("-", "") for node in total_pairs.loc[:, "Starting_node"]
    ]
    total_pairs.loc[:, "Connecting_node"] = [
        node.replace("+", "") for node in total_pairs.loc[:, "Connecting_node"]
    ]
    total_pairs.loc[:, "Connecting_node"] = [
        node.replace("-", "") for node in total_pairs.loc[:, "Connecting_node"]
    ]
    ...
    complete_node_info = pd.DataFrame(
        columns=["Starting_node", "Connecting_node", "weight"]
    )
    for node in sorted(list(set(total_pairs.loc[:, "Starting_node"]))):
        ...
        particular_node = pd.DataFrame(
            particular_node, columns=["Starting_node", "Connecting_node", "weight"]
        )
        particular_node.loc[:, "scaled_weight"] = scalar1(
            particular_node.loc[:, "weight"]
        )  # add a column with scaled weights using scalar1()
        complete_node_info = pd.concat(
            [complete_node_info, particular_node], ignore_index=True
        )
    ...
    if total_pairs.shape[0] > 0 and total_pairs.shape[1] > 0:
        ...
        if pl_unbinned.shape[0] > 0:
            ...
            pl_unbinned.loc[:, "Prob_Chromosome"] = round(
                pl_unbinned.loc[:, "Prob_Chromosome"], 2
            )
            pl_unbinned.loc[:, "Prob_Plasmid"] = round(
                pl_unbinned.loc[:, "Prob_Plasmid"], 2
            )
            pl_unbinned.loc[:, "coverage"] = round(pl_unbinned.loc[:, "coverage"], 2)

        with open(path_nodes) as file:
            raw_nodes = [
                [str(values[0]), str(values[1])] for values in SimpleFastaParser(file)
            ]
        ...
        results_subgraph.to_csv(
            output_components, sep="\t", index=False, header=True, mode="w"
        )
        return False
    ...
    weight_graph = pd.DataFrame(
        data={
            "From_to": [pair[0] for pair in pairs],
    ...
    node_and_component = pd.DataFrame(
        data={
            "Node": [
                no_loops_graph.vs[index]["name"]
                for index in range(len(no_loops_graph.vs))
            ],
            "Original_component": info_comp_member,
        }
    )

    information_components = pd.DataFrame(
        data={"Original_component": original_components, "Size": info_comp_size}
    )

    full_info_components = node_and_component.merge(
        information_components, on="Original_component"
    )
    ...
    if len(components_graph) >= 1:
        ...
        for component in range(len(components_graph)):
            ...
            partition_info.loc[:, "Original_component"] = info_first_node.loc[
                :, "Original_component"
            ].values[0]
            complete_partition_info = pd.concat(
                [complete_partition_info, partition_info], ignore_index=True
            )

        complete_partition_info.loc[:, "Modularity"] = round(
            complete_partition_info.loc[:, "Modularity"], 2
        )

        # Get the decision of splitting or not. if the modularity value is bigger than the threshold, split. Otherwise, don't.
        complete_partition_info.loc[:, "Decision"] = [
            "Split" if score >= modularity_threshold else "No_split"
            for score in complete_partition_info.loc[:, "Modularity"]
        ]
    ...
    singletons_component = [
        i for i, x in enumerate(index) if x
    ]  # TODO replace this list comprehension with np.where?

    # Add the singletons to the rest of the data.
    if len(singletons_component) > 0:
        df_singletons = pd.DataFrame(
            data={
                "Algorithm": "Independent_single_component",
                "Modularity": 0,
        ...
        complete_partition_info = pd.concat(
            [complete_partition_info, df_singletons], ignore_index=True
        )

    complete_partition_info.sort_values(
        by="Original_component", axis=0, ascending=True, inplace=True, ignore_index=True
    )
    ...
    for component in sorted(
        list(set(complete_partition_info.loc[:, "Original_component"]))
    ):
        ...
        if decision_comp.iloc[0, 0] != "Independent_single_component":
            ...
            no_split_decision = sum(
                decision_comp.loc[:, "Decision"].str.count("No_split")
            )

            if split_decision >= no_split_decision:
                index = decision_comp.loc[:, "Modularity"] == max(
                    decision_comp.loc[:, "Modularity"]
                )
                ...
                for node in range(len(spl_names)):
                    contigs_membership.append(
                        [algorithm, component, spl_membership[node], spl_names[node]]
                    )
                ...
                for node in range(
                    nodes_component.shape[0]
                ):  # TODO check if this 0 from membership is always 0?
                    contigs_membership.append(
                        [
                            "Not_split_component",
                            component,
                            0,
                            nodes_component.iloc[node, 0],
                        ]
                    )
    ...
    contigs_membership = pd.DataFrame(
        data=contigs_membership,
        columns=["Algorithm", "Original_component", "Bin", "Contig"],
    )
    contigs_membership.loc[:, "Cluster"] = [
        "-".join(
            [
                str(contigs_membership.loc[row, "Original_component"]),
                str(contigs_membership.loc[row, "Bin"]),
            ]
        )
        for row in range(contigs_membership.shape[0])
    ]
    contigs_membership.sort_values(
        by="Cluster", axis=0, ascending=True, inplace=True, ignore_index=True
    )
    contigs_membership.loc[:, "Final_cluster"] = contigs_membership.loc[
        :, "Cluster"
    ].copy()
    ...
    for number in range(len(unique_clusters)):
        ...
        contigs_membership.loc[:, "Final_cluster"] = [
            cluster.replace(cluster_name, str(number))
            for cluster in contigs_membership.loc[:, "Final_cluster"]
        ]
    ...
    contigs_membership.loc[:, "Color"] = [
        set_colors[int(cluster)]
        for cluster in contigs_membership.loc[:, "Final_cluster"]
    ]
    ...
    results_subgraph = pd.DataFrame(
        data={
            "number": contigs_membership.loc[:, "Contig"],
    ...
    index = clean_pred.loc[:, "Prob_Plasmid"] >= pred_threshold
    pl_nodes = clean_pred.loc[
        index, :
    ].copy()  # Selecting only contigs predicted as plasmid-derived
    ...
    isolated_nodes = isolated_nodes.astype(
        {
            "Prob_Chromosome": float,
            "Prob_Plasmid": float,
            "Prediction": str,
    ...
    if pl_unbinned.shape[0] >= 1:
        pl_unbinned.loc[:, "Component"] = "Unbinned"
        full_info_assigned = pd.concat(
            [full_info_assigned, pl_unbinned], ignore_index=True
        )
    ...
    if pl_isolated.shape[0] >= 1:
        ...
        full_info_assigned = pd.concat(
            [full_info_assigned, pl_isolated], ignore_index=True
        )
    ...
    if pl_repeats.shape[0] >= 1:
        ...
        full_info_assigned = pd.concat(
            [full_info_assigned, pl_repeats], ignore_index=True
        )

    # Add information to the results file
    full_info_assigned = full_info_assigned.drop(columns="Contig_length")
    full_info_assigned.loc[:, "Prob_Chromosome"] = round(
        full_info_assigned.loc[:, "Prob_Chromosome"], 2
    )
    full_info_assigned.loc[:, "Prob_Plasmid"] = round(
        full_info_assigned.loc[:, "Prob_Plasmid"], 2
    )
    full_info_assigned.loc[:, "coverage"] = round(
        full_info_assigned.loc[:, "coverage"], 2
    )
    ...
    full_info_assigned.to_csv(
        output_results, sep="\t", index=False, header=True, mode="w"
    )
    results_subgraph.to_csv(
        output_components, sep="\t", index=False, header=True, mode="w"
    )
