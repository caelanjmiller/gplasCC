...
if(manual_mode == True):
    ...
    path_init_nodes = "coverage\\ecoli_initialize_nodes.tab"
    path_isolated_nodes = "coverage\\ecoli_isolated_nodes.tab"
    path_cov_variation = "coverage\\ecoli_estimation.txt"
    input_solutions = "walks\ecoli_solutions.tab"
    #Params
    classifier = "predict"
    ...
    #Outputs
    output_dir = "results\\"
    output_results = "results\\ecoli_results_no_repeats.tab"
    output_components = "results\\ecoli_bins_no_repeats.tab"
    output_png = "results\\ecoli_plasmidome_network.png"
else:
    #Inputs
    ...
    path_init_nodes = str(snakemake.input["initialize_nodes"])
    path_isolated_nodes = str(snakemake.input["isolated_nodes"])
    path_cov_variation = str(snakemake.input["coverage"])
    input_solutions = str(snakemake.input["solutions"])
    #Params
    ...
    output_png = str(snakemake.output["plot_graph"])
...
#improve remove this empty png?? or keep for consistency by always producing 'a' plot?
    ...
    #ASK is sys.exit() the right function to use?
    sys.exit(0)
