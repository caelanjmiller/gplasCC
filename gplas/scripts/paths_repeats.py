...
def generate_repeat_paths(sample, number_iterations, filtering_threshold, outdir):
    ...
    small_contigs.loc[:, "number"] = [
        name.replace("+", "") for name in small_contigs["number"]
    ]
    small_contigs.loc[:, "number"] = [
        name.replace("-", "") for name in small_contigs["number"]
    ]
    ...
    def plasmid_graph(
        ...
        for iteration in range(
            number_iterations
        ):  # Number of times we repeat this process
            path = [
                initial_seed
            ]  # We add the initial seed to the path, first element in the list
            seed = initial_seed
            ...
            info_path = graph_contigs[
                [number in path for number in graph_contigs["number"]]
            ].copy()
            length_path = sum(info_path["length"])  # Length of the path
            info_path["contribution"] = (
                info_path["length"] / length_path
            )  # Shorter contigs should have a lower contribution to the coverage. k-mer coverage on these contigs fluctuates drastically
            path_mean = np.average(
                info_path["coverage"], weights=info_path["contribution"]
            )  # Coverage of the current path
            ...
            for elongation in range(number_nodes):
                ...
                current_links = links.loc[
                    index, :
                ]  # Consider the last element present in our path and observe all the possible links

                if current_links.shape[0] == 0:
                    output = ",".join(
                        path
                    )  # join path with ',' for later step in coocurrence_repeats script
                    output = "\t".join(
                        [
                            output,
                            classification,
                            unitig_seed_classification,
                            str(path_mean),
                        ]
                    )
                    ...
                    path = [
                        initial_seed
                    ]  # There are no connections possible from this contig
                    unitig_seed_classification = classification
                    break  # Exiting elongation loop

                list_connections = sorted(
                    list(set(current_links[2]))
                )  # All the possible unique connections
                ...
                if (len(path) > 1) | (
                    ...
                    ommit_nodes = [
                        node for node in ommit_nodes if node not in repeats_signed_nodes
                    ]

                    list_connections = [
                        str(connection)
                        for connection in list_connections
                        if connection not in ommit_nodes
                    ]  # Avoiding loops within the solution

                total_connections = len(list_connections)  # Number of connections
                base_probabilities = (
                    [float(0)] * total_connections
                )  # Generating a list with the connection probabilities
                ...
                prob_df.loc[:, "number"] = [
                    number.replace("+", "") for number in prob_df["number"]
                ]
                prob_df.loc[:, "number"] = [
                    number.replace("-", "") for number in prob_df["number"]
                ]
                ...
                if (
                    (classification != "Plasmid") & (classification != "Chromosome")
                ) | (
                    (unitig_seed_classification != "Plasmid")
                    & (unitig_seed_classification != "Chromosome")
                ):
                    ...
                    for number in prob_df.loc[:, "number"]:
                        ...
                        if any(matchID):
                            ...
                            prob_df.loc[index, "Prob_Plasmid"] = float(
                                clean_pred.loc[matchID, "Prob_Plasmid"].values[0]
                            )
                            prob_df.loc[index, "Prob_Chromosome"] = float(
                                clean_pred.loc[matchID, "Prob_Chromosome"].values[0]
                            )
                ...
                record_connections = pd.DataFrame(
                    data=record_connections,
                    columns=[
                        "factor",
                        "number_iterations",
                        "iteration",
                ...
                index = graph_contigs.loc[:, "number"].isin(
                    record_connections.loc[:, "outgoing_node"]
                )
                ...
                record_connections = record_connections.merge(
                    cov_connections_info[["number", "Probability_cov"]],
                    left_on="outgoing_node",
                    right_on="number",
                )
                record_connections = record_connections.drop(columns="number")

                index = record_connections.loc[:, "outgoing_node"].isin(
                    repeats_signed_nodes
                )
                record_connections.loc[index, "Probability_cov"] = float(
                    prob_small_repeats
                )

                index = record_connections.loc[:, "outgoing_node"].isin(
                    small_contigs_signed_nodes
                )
                record_connections.loc[index, "Probability_cov"] = float(
                    prob_small_repeats
                )

                record_connections.loc[:, "Probability"] = (
                    record_connections.loc[:, "Probability_pl_chr"]
                    * record_connections.loc[:, "Probability_cov"]
                )
                record_connections.loc[:, "Probability_freq"] = record_connections.loc[
                    :, "Probability"
                ] / sum(record_connections.loc[:, "Probability"])
                record_connections.loc[:, "Verdict"] = "non-selected"

                if (
                    sum(record_connections.loc[:, "Probability"] >= filtering_threshold)
                    == 0
                ):
                    output = ",".join(
                        path
                    )  # We join path with ',' for later step in coocurrence_repeats script
                    output = "\t".join(
                        [
                            output,
                            classification,
                ...
                index = list(
                    record_connections.loc[:, "Probability"] >= filtering_threshold
                )
                filter_connections = record_connections.loc[index, :].copy()
                if (
                    sum(filter_connections["Probability_freq"]) != 1
                ):  # recalculate probability frequencies after filter
                    filter_connections["Probability_freq"] = filter_connections.loc[
                        :, "Probability"
                    ] / sum(filter_connections.loc[:, "Probability"])

                random_connection = np.random.choice(
                    filter_connections.loc[:, "outgoing_node"],
                    size=1,
                    p=filter_connections.loc[:, "Probability_freq"],
                )[0]  # Choose one connection
                ...
                if (random_connection == path[0]) & (classification == "Plasmid"):
                    output = ",".join(
                        path
                    )  # We join path with ',' for a later step in coocurrence_repeats script
                    output = "\t".join(
                        [
                            output,
                            classification,
                ...
                if (
                    (unitig_seed_classification == "Repeat")
                    | (unitig_seed_classification == "Small")
                ) & (sum(clean_pred.loc[:, "number"].isin([seed_clean])) > 0):
                    unitig_seed_info = clean_pred.loc[
                        clean_pred.loc[:, "number"].isin([seed_clean]), :
                    ]
                    unitig_seed_classification = unitig_seed_info.loc[
                        :, "Prediction"
                    ].values[0]
                elif (unitig_seed_classification == "Repeat") & (
                    sum(info_path.loc[:, "number"].isin(repeats_signed_nodes))
                    != info_path.shape[0]
                ):
                    unitig_seed_classification = "Small"
                ...
                if unitig_seed_classification != "Repeat":
                    ...
                    info_path.loc[:, "contribution"] = (
                        info_path.loc[:, "length"] / length_path
                    )
                    path_mean = np.average(
                        info_path.loc[:, "coverage"],
                        weights=info_path.loc[:, "contribution"],
                    )  # Coverage of the current path

                    if (unitig_seed_classification == "Plasmid") | (
                        unitig_seed_classification == "Chromosome"
                    ):
                        output = ",".join(
                            path
                    ...
                    first_node = info_path.loc[
                        info_path.loc[:, "number"].isin([initial_seed])
                    ].copy()
                    index = ~info_path.loc[:, "number"].isin(repeats_signed_nodes)
                    info_path = info_path.loc[index, :]
