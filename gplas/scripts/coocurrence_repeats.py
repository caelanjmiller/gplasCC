...
def scalar1(
    x,
):  # TODO can we do something like "from coocurrence.py import scalar1"? it is the same identical function
...
def calculate_coocurrence_repeats(sample, outdir, sd_coverage=2):
    ...
    # merge it
    solutions = solutions.merge(
        bins_data.loc[:, ["number", "Bin"]],
        how="left",
        left_on="last_nodes_signless",
        right_on="number",
    )
    ...
    solutions.loc[:, "repeat_bin"] = [
        "-".join([solutions.loc[row, "initial_node"], solutions.loc[row, "Bin"]])
        for row in range(solutions.shape[0])
    ]
    ...
    solutions.loc[:, "keep"] = [
        "yes"
        if (
            (solutions.loc[row, "step_1"] == solutions.loc[row, "last_nodes"])
            & (solutions.loc[row, "unitig_classification"] != "Repeat")
        )
        else "no"
        for row in range(solutions.shape[0])
    ]
    ...
    index = [
        solutions.loc[row, "step_1"][-1] == "+" for row in range(solutions.shape[0])
    ]
    ...
    index = (
        positive_solutions.loc[:, "repeat_bin"].isin(
            negative_solutions.loc[:, "repeat_bin"]
        )
    ) & (positive_solutions.loc[:, "keep"] == "no")
    positive_solutions_keep = positive_solutions.loc[index, :]
    index = (
        negative_solutions.loc[:, "repeat_bin"].isin(
            positive_solutions.loc[:, "repeat_bin"]
        )
    ...
    solutions = pd.concat(
        [final_walks, final_positive_walks, final_negative_walks], ignore_index=True
    )
    ...
    starting_nodes = [
        node for node in unique_nodes if node in solutions.loc[:, "step_0"].values
    ]
    ...
    complete_node_info = pd.DataFrame(
        columns=["Starting_node", "Connecting_node", "weight", "scaled_weight"]
    )
    for node in sorted(list(set(total_pairs.loc[:, "Starting_node"]))):
        ...
        if (
            complete_node_info.shape[0] == 0
        ):  # if statement to prevent "FutureWarning: concatenation with empty DataFrame"
    ...
    total_pairs = total_pairs.merge(
        bins_data.loc[:, ["number", "Bin"]],
        how="left",
        left_on="Connecting_node",
        right_on="number",
    )
    ...
    total_pairs = total_pairs.loc[
        :, ["Starting_node", "Bin", "weight", "scaled_weight"]
    ]
    ...
    if total_pairs.shape[0] > 0 and total_pairs.shape[1] > 0:
        total_pairs.loc[:, "Pair"] = [
            "-".join(
                [total_pairs.loc[row, "Bin"], total_pairs.loc[row, "Starting_node"]]
            )
    ...
    weight_graph = pd.DataFrame(
        data={
            "From_to": [pair[1] for pair in pairs],
    ...
    clean_repeats = clean_repeats.astype({"number": str, "coverage": float})
    ...
    weight_graph = weight_graph.merge(
        clean_repeats.loc[:, ["number", "coverage"]],
        how="left",
        left_on="From_to",
        right_on="number",
    )
    ...
    weight_graph = weight_graph.merge(
        bins_coverage, how="left", left_on="To_from", right_on="Bin"
    )
    ...
    repeat_assignments = pd.DataFrame(
        columns=["From_to", "To_from", "weight", "coverage", "bin_coverage", "rank"]
    )
    ...
    for node in sorted(list(set(weight_graph.loc[:, "From_to"]))):
        ...
        df_node.sort_values(
            by="weight", axis=0, ascending=False, ignore_index=True, inplace=True
        )
        ...
        while rank <= max(df_node.loc[:, "rank"]):
            ...
            if (
                repeat_bin.loc[:, "coverage"].values[0]
                + max_variation
                - accumulated_cov
                >= repeat_bin.loc[:, "bin_coverage"].values[0]
            ):
                accumulated_cov += repeat_bin.loc[:, "bin_coverage"].values[0]
                if (
                    repeat_assignments.shape[0] == 0
                ):  # if statement to prevent "FutureWarning: concatenation with empty DataFrame"
                    repeat_assignments = repeat_bin
                else:
                    repeat_assignments = pd.concat(
                        [repeat_assignments, repeat_bin], ignore_index=True
                    )
            rank += 1

    repeat_assignments = repeat_assignments.loc[:, ["To_from", "From_to"]].rename(
        columns={"To_from": "Bin", "From_to": "number"}
    )
    ...
    if plasmid_repeats.shape[0] == 0:
        print(
            "gplas did not find repeated elements associated with plasmid predictions"
        )
        bins_data.loc[:, "Prob_Chromosome"] = round(
            bins_data.loc[:, "Prob_Chromosome"], 2
        )
        ...
        index = clean_repeats.loc[:, "number"].isin(
            plasmid_repeats.loc[:, "number"]
        )  # Selecting only contigs predicted as plasmid-derived
        ...
        full_info_assigned = pd.concat(
            [full_info_assigned, bins_data], ignore_index=True
        )
    ...
    chromosome_repeats = chromosome_repeats.loc[
        :, ["number", "Bin"]
    ]  # TODO if possible change column order from the start instead of changing it at the end
    chromosome_repeats.loc[:, "Bin"] = (
        "Chromosome"  # TODO if possible call the column name 'Chromosome' from the start instead of 'C' and renaming it at the end
    )
    chromosome_repeats.to_csv(
        output_chromosomes, sep="\t", index=False, header=True, mode="w"
    )
