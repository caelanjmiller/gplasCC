...
if(manual_mode == True):
    ...
    output_connections = "walks\\normal_mode\\ecoli_connections.tab"
else:
    #Inputs
...
"""  
#R match() equivalent?          
forw = pd.Series([1,2,3,4,5,6,7])
reve = pd.Series([7,6,5,4,3,2,1])
[reve.index(x) if x in reve else None for x in forw]
idx=[reve.index[reve==forw[3]].to_list()[0] if forw.isin(reve)[i] == True else np.nan for i in range(len(forw))]
pd.Index(idx)
reve.loc[idx]
"""