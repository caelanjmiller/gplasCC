...
if(manual_mode == True):
    ...
    isolate_name = arguments[0]
    prediction_dir = arguments[1]

#get a path for the prediction file
prediction_file_path = prediction_dir
#get a path for fasta file.
raw_nodes_path = "".join(["gplas_input/", isolate_name, "_raw_nodes.fasta"]) 

#Check if prediction file exists
if(os.path.exists(prediction_file_path) == False):
...
prediction_file = pd.read_csv(prediction_file_path, sep="\t", header=0)
