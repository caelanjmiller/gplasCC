...
from gplas.scripts.check_prediction_format import (
    check_prediction,
...
parser = argparse.ArgumentParser(
    description="gplasCC: A tool for binning plasmid-predicted contigs into individual predictions",
    add_help=False,
)
...
inputgroup.add_argument(
    ...
    required=True,
    help="Path to the graph file in GFA (.gfa) format, used to extract nodes and links",
)
inputgroup.add_argument(
    "-o", dest="outdir", type=utils.is_valid_dir, default=".", help="Output directory"
)  # TODO go through all scripts and add outdir where needed
inputgroup.add_argument(
    "-n",
    dest="name",
    type=str,
    help="Name prefix for output files (default: input file name)",
)

classifiergroup = inputgroup.add_mutually_exclusive_group(required=True)
classifiergroup.add_argument(
    "-s",
    dest="species",
    type=utils.check_species,
    help="Choose a species database for plasmidCC classification. Use --speciesopts for a list of all supported species",
)
classifiergroup.add_argument(
    "-p",
    dest="custom_db_path",
    type=utilsCC.verify_user_db,
    help="Path to a custom Centrifuge database (name without file extensions)",
)
classifiergroup.add_argument(
    "-P",
    dest="prediction",
    type=utils.file_exists,
    help="If not using plasmidCC. Provide a path to an independent binary classification file",
)
classifiergroup.add_argument(
    "--extract",
    action="store_true",
    help="extract FASTA sequences from the assembly graph to use with an external classifier",
)
...
paramgroup.add_argument(
    ...
    type=float,
    default=0.5,
    help="Prediction threshold for plasmid-derived sequences (default: %(default)s)",
)
paramgroup.add_argument(
    "-b",
    dest="bold_coverage_sd",
    type=int,
    default=5,
    help="Coverage variance allowed for bold walks to recover unbinned plasmid-predicted nodes (default: %(default)s)",
)
paramgroup.add_argument(
    "-r",
    dest="repeats_coverage_sd",
    type=int,
    default=2,
    help="Coverage variance allowed for assigning repeats to bins (default: %(default)s)",
)
paramgroup.add_argument(
    "-x",
    ...
    type=int,
    default=20,
    help="Number of walk iterations per starting node (default: %(default)s)",
)
paramgroup.add_argument(
    "-f",
    dest="filt_gplas",
    type=float,
    default=0.1,
    help="filtering threshold to reject outgoing edges (default: %(default)s)",
)
paramgroup.add_argument(
    "-e",
    dest="edge_threshold",
    type=float,
    default=0.1,
    help="Edge threshold (default: %(default)s)",
)
paramgroup.add_argument(
    "-q",
    dest="modularity_threshold",
    type=float,
    default=0.2,
    help="Modularity threshold to split components in the plasmidome network (default: %(default)s)",
)
paramgroup.add_argument(
    "-l",
    dest="length_filter",
    type=int,
    default=1000,
    help="Filtering threshold for sequence length (default: %(default)s)",
)

othergroup = parser.add_argument_group("Other")
othergroup.add_argument(
    "-k", "--keep", action="store_true", help="Keep intermediary files"
)
...
infogroup.add_argument(
    ...
    action="printing",
    nargs=0,
    help="Prints a list of all supported species for the -s flag",
)
infogroup.add_argument(
    "-v", "--version", action="printing", nargs=0, help="Prints gplas version"
)
infogroup.add_argument(
    "-h", "--help", action="printing", nargs=0, help="Prints this message"
)
...
if not args.extract:
    ...
    print(
        "Threshold for predicting plasmid-derived contigs:.....",
        args.threshold_prediction,
    )
    print(
        "Number of plasmid walks created per node:.............", args.number_iterations
    )
    print("Threshold of gplas scores:............................", args.filt_gplas)
    print("Minimum frequency to consider an edge:................", args.edge_threshold)
    print(
        "Modularity threshold used to partition the network:...",
        args.modularity_threshold,
    )
    print(
        "Coverage SD for bold mode:............................", args.bold_coverage_sd
    )
...
if not line_content:
    print(
        "There are no suitable plasmids to initiate a random walk. gplas can't do anything"
    )
    if not args.keep:
...
generate_paths(
    sample, args.number_iterations, args.filt_gplas, outdirname, mode="normal"
)
...
if not calculate_coocurrence(
    ...
    print(
        "Plasmid nodes will be classified as Unbinned. If this is unexpected, please assemble your genome with different parameters or with a different tool and re-run gplas"
    )
else:
    print("Calculating coocurrence of random walks............... completed!")
...
if os.path.exists(unbinned_path):
    ...
    print(
        "Some contigs were left unbinned"
    )  # improve tell user how many contigs are unbinned?
    ...
    if not calculate_coocurrence(
        ...
        print(
            "gplas bold mode couldn't find any walks connecting plasmid-predicted nodes"
        )
        print(
            "Plasmid nodes will be classified as Unbinned. If this is unexpected, please assemble your genome with different parameters or with a different tool and re-run gplas"
        )
...
if line_content:
    ...
    if not calculate_coocurrence_repeats(sample, outdirname, args.repeats_coverage_sd):
        ...
        shutil.move(
            f"{outdirname}/results/{sample}_results_no_repeats.tab",
            f"results/{sample}_results.tab",
        )
        shutil.move(
            f"{outdirname}/results/{sample}_bins_no_repeats.tab",
            f"results/{sample}_bins.tab",
        )
...
# _5.1.2 If there are no repeated elements, just rename the results files
    ...
    shutil.move(
        f"{outdirname}/results/{sample}_results_no_repeats.tab",
        f"{outdirname}/results/{sample}_results.tab",
    )
    shutil.move(
        f"{outdirname}/results/{sample}_bins_no_repeats.tab",
        f"{outdirname}/results/{sample}_bins.tab",
    )
