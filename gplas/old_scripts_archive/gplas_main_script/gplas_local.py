...
pkgdir = os.path.dirname("C:/Users/oscar/Documenten/UU/04_BiBc/6.2_Research_Profile/gplas2-master/gplas")
...
template_file=f'templates/oscar_assembly.yaml'
...
with open(template_file, 'w+') as template:
    ...
    template.write(f'  "oscar": "C:/Users/oscar/Documenten/UU/04_BiBc/6.2_Research_Profile/gplas2-master/test/test_ecoli.gfa"\n')
    template.write(f'threshold_prediction: "0.5"\n')
    template.write(f'number_iterations: "20"\n')
    template.write(f'classifier: "predict"\n')
    template.write(f'filt_gplas: "0.1"\n')
    template.write(f'edge_gplas: "0.1"\n')
    template.write(f'name: "oscar"\n')
    template.write(f'modularity_threshold: "0.2"\n')
    template.write(f'bold_sd_coverage: "5"\n')
    template.write(f'min_node_length: "1000"\n')
    template.write(f'repeats_coverage_sd: "2"\n')
    template.write(f'predict_dir: "C:/Users/oscar/Documenten/UU/04_BiBc/6.2_Research_Profile/gplas2-master/gplas/independent_prediction/test_ecoli_plasmid_prediction.tab"\n')
...
if args.classifier=='extract':
    ...
    command_snakemake_unlock=f'snakemake --unlock --use-conda --configfile {template_file} -d $PWD -s {snakeFile} gplas_input/oscar_raw_nodes.fasta'
    command_snakemake_run=f'snakemake --use-conda --configfile {template_file} -d $PWD -s {snakeFile} gplas_input/oscar_raw_nodes.fasta'
