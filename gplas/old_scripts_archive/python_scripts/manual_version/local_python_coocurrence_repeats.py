...
path_init_nodes = "test_run\\coverage\\ecoli_repeat_nodes.tab"
path_cov_variation = "test_run\\coverage\\ecoli_estimation.txt"
input_solutions = "test_run\\walks\\repeats\\ecoli_solutions.tab"
path_bins = "test_run\\results\\ecoli_results_no_repeats.tab"
clean_repeats_path = "test_run\\coverage\\ecoli_clean_repeats.tab"
...
#TODO repeats are not assigned -1 in R?
...
if((total_pairs.shape[0] > 0) & (total_pairs.shape[1] > 0)):
    ...
    #TODO is sys.exit() the right function to use?; why is it randomly status=1 in the final script?
    sys.exit(1)
...
#TODO this whole rank stuff was just for fun and is immediatly removed again????????????????
...
full_info_assigned.to_csv("test_run\\results\\ecoli_results.tab", sep="\t", index=False, header=True, mode="a")
results_summary.to_csv("test_run\\results\\ecoli_bins.tab", sep="\t", index=False, header=True, mode="a")
...
chromosome_repeats.to_csv("test_run\\results\\ecoli_chromosome_repeats.tab", sep="\t", index=False, header=True, mode="a")
