...
path_links = "test_run\\gplas_input\\ecoli_raw_links.txt"
classifier = "predict"
threshold = float("0.95")
path_prediction = "gplas\\independent_prediction\\test_ecoli_plasmid_prediction.tab"
...
graph_contigs.to_csv("test_run\\coverage\\ecoli_graph_contigs.tab", sep="\t", index=False)
...
links.to_csv("test_run\\coverage\\ecoli_clean_links.tab", sep="\t", index=False, header=False)
...
repeats.to_csv("test_run\\coverage\\ecoli_repeats_graph.tab", sep="\t", index=False)
...
final_prediction.to_csv("test_run\\coverage\\ecoli_clean_prediction.tab", sep="\t", index=False)
...
isolated_nodes.to_csv("test_run\\coverage\\ecoli_isolated_nodes.tab", sep="\t", index=False)

repeats_final = clean_pred[[number in list(repeats["number"]) for number in clean_pred["number"]]]

repeats_final.to_csv("test_run\\coverage\\ecoli_clean_repeats.tab", sep="\t", index=False)
...
with open("test_run\\coverage\\ecoli_initialize_nodes.tab", "w") as file:
    file.write("\n".join(initialize_nodes))

repeats_nodes = list(set(repeats_final["number"]))

with open("test_run\\coverage\\ecoli_repeat_nodes.tab", "w") as file:
    file.write("\n".join(repeats_nodes))
...
with open("test_run\\coverage\\ecoli_estimation.txt", "w") as file:
