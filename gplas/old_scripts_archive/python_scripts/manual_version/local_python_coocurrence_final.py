...
path_isolated_nodes = "test_run\\coverage\\ecoli_isolated_nodes.tab"
path_cov_variation = "test_run\\coverage\\ecoli_estimation.txt"
input_solutions = "test_run\\walks\ecoli_solutions.tab"
...
#Outputs
# fill in later to replace hardcoded stuff
# ctrl+f filename
output_dir = "test_run\\results\\"
output_png = "test_run\\results\\ecoli_plasmidome_network.png"
...
#TODO filter is on weight > 1 why not weight > 0??
...
if((total_pairs.shape[0] > 0) & (total_pairs.shape[1] > 0)): # improve remove the second redundant conditional (also other scripts)
    ...
    index = clean_pred.loc[:,"Prob_Plasmid"] > threshold
    ...
    for component in set(df_nodes.loc[:,"Component"]):
        ...
        component_complete_name = "_".join(["ecoli", "bin", str(component)])
...
#can we use lowercase 'bin'?
    ...
    #TODO unhardcode these filenames
    pl_unbinned.to_csv("test_run\\results\\ecoli_results_no_repeats.tab", sep="\t", index=False, header=True, mode="w")
    results_subgraph.to_csv("test_run\\results\\ecoli_bins_no_repeats.tab", sep="\t", index=False, header=True, mode="w")
#improve remove this empty png?? or keep for consistency by always producing 'a' plot?
    ...
    #TODO is sys.exit() the right function to use?
    sys.exit(0)
...
#TODO mistake in R?? pl_isolated is pl_unbinned, filtered with the index of pl_notassigned; probem is copied and present in .py
...
#TODO unhardcode these filenames
full_info_assigned.to_csv("test_run\\results\\ecoli_results_no_repeats.tab", sep="\t", index=False, header=True, mode="a")
results_subgraph.to_csv("test_run\\results\\ecoli_bins_no_repeats.tab", sep="\t", index=False, header=True, mode="a")
