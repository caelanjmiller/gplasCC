...
if(manual_mode == True):
    ...
    os.getcwd()
    #Inputs
    path_nodes = "gplas_input\\ecoli_spades_raw_nodes.fasta"
    path_links = "gplas_input\\ecoli_spades_raw_links.txt"
    path_prediction = "ecoli_spades_plasmid_prediction.tab"
    #Params
    classifier = "predict"
    threshold = float("0.5")
    #Outputs
...
if(kc_check == len(raw_contig_names)):
  ...
  kc_coverage = [kc/length for kc in kc_count]
