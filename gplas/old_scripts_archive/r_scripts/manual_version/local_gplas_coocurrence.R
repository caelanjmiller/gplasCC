...
# Inputs
path_nodes <- "./test_run_R/gplas_input/ecoli_raw_nodes.fasta"
path_links <- "./test_run_R/coverage/ecoli_clean_links.tab"
path_prediction <- "./test_run_R/coverage/ecoli_clean_prediction.tab"
path_graph_contigs <- "./test_run_R/coverage/ecoli_graph_contigs.tab"
path_graph_repeats <- "./test_run_R/coverage/ecoli_repeats_graph.tab"
path_init_nodes <- "./test_run_R/coverage/ecoli_initialize_nodes.tab"
path_isolated_nodes <- "./test_run_R/coverage/ecoli_isolated_nodes.tab"
path_cov_variation <- "./test_run_R/coverage/ecoli_estimation.txt"
input_solutions <- "./test_run_R/walks/normal_mode/ecoli_solutions.csv"
classifier <- "predict"
threshold <- "0.95"
iterations <- "20"
...
for(row in 1:nrow(total_pairs))
{
  ...
  for(component in unique(df_nodes$Component))
  {
    ...
    component_complete_name <- paste("ecoli", 'bin', sep = '_')
    filename <- paste('./test_run_R/results/normal_mode/', component_complete_name, sep = '')
  ...
  suppressWarnings(write.table(x = pl_unassigned, file = "./test_run_R/results/normal_mode/ecoli_results_no_repeats.tab", append = TRUE, row.names = FALSE, quote = FALSE, col.names = TRUE))
  suppressWarnings(write.table(x = results_subgraph, file = "./test_run_R/results/normal_mode/ecoli_bins_no_repeats.tab", append = TRUE, row.names = FALSE, quote = FALSE, col.names = TRUE))
  
  png(filename="./test_run_R/results/normal_mode/ecoli_plasmidome_network.png",width = 700, height = 700)
  plot.new()
  dev.off()
...
suppressWarnings(write.table(x = full_info_assigned, file = "./test_run_R/results/normal_mode/ecoli_results_no_repeats.tab", append = TRUE, row.names = FALSE, quote = FALSE, col.names = TRUE))
suppressWarnings(write.table(x = results_subgraph, file = "./test_run_R/results/normal_mode/ecoli_bins_no_repeats.tab", append = TRUE, row.names = FALSE, quote = FALSE, col.names = TRUE))
