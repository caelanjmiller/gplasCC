...
input_solutions <- "./test_run_R/walks/ecoli_solutions.csv"
classifier <- "predict"
threshold <- "0.95"
iterations <- "20"
...
if (nrow(total_pairs) == 0) {
  ...
  for(component in unique(df_nodes$Component))
  {
    ...
    filename <- paste('./test_run_R/results/', component_complete_name, sep = '')
  ...
  suppressWarnings(write.table(x = pl_unassigned, file = "./test_run_R/results/ecoli_results_no_repeats.tab", append = TRUE, row.names = FALSE, quote = FALSE, col.names = TRUE))
  suppressWarnings(write.table(x = results_subgraph, file = "./test_run_R/results/ecoli_bins_no_repeats.tab", append = TRUE, row.names = FALSE, quote = FALSE, col.names = TRUE))
  
  png(filename="./test_run_R/results/ecoli_plasmidome_network.png",width = 700, height = 700)
  plot.new()
  dev.off()
  quit(status=0)
} else {
...
suppressWarnings(write.table(x = full_info_assigned, file = "./test_run_R/results/ecoli_results_no_repeats.tab", append = TRUE, row.names = FALSE, quote = FALSE, col.names = TRUE))
suppressWarnings(write.table(x = results_subgraph, file = "./test_run_R/results/ecoli_bins_no_repeats.tab", append = TRUE, row.names = FALSE, quote = FALSE, col.names = TRUE))
