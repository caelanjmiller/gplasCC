...
path_links <- "./test_run_R/gplas_input/ecoli_raw_links.txt"
classifier <- "predict"
threshold <- "0.95"
path_prediction <- "./gplas/independent_prediction/test_ecoli_plasmid_prediction.tab"
...
write.table(x = graph_contigs, file = "./test_run_R/coverage/ecoli_graph_contigs.tab", row.names = FALSE)
...
write.table(x = links, file = "./test_run_R/coverage/ecoli_clean_links.tab", row.names = FALSE)
...
write.table(x = repeats_graph, file = "./test_run_R/coverage/ecoli_repeats_graph.tab")
...
write.table(x = final_prediction, file = "./test_run_R/coverage/ecoli_clean_prediction.tab", row.names = FALSE)
...
write.table(x = isolated_nodes, file = "./test_run_R/coverage/ecoli_isolated_nodes.tab", row.names = FALSE)

#Get the repeat information.
repeats_final <- clean_pred[clean_pred$number %in% repeats$number,]
write.table(x = repeats_final, file = "./test_run_R/coverage/ecoli_clean_repeats.tab", row.names = FALSE)
...
write.table(x = initialize_nodes, file = "./test_run_R/coverage/ecoli_initialize_nodes.tab", row.names = FALSE)

###########################################################################################################################################

# Select the repeat nodes that will be used to explore their surroundings
repeats_nodes<-unique(repeats_final$number)
write.table(x = repeats_nodes, file = "./test_run_R/coverage/ecoli_repeat_nodes.tab", row.names = FALSE)
...
write.table(x = sd_estimation, file = "./test_run_R/coverage/ecoli_estimation.txt", row.names = FALSE)
...
stats::mad(cov_estimation$coverage, constant = 1)
BiocGenerics::mad(cov_estimation$coverage)
median(cov_estimation$coverage)

1.4826 * median(abs(cov_estimation$coverage - median(cov_estimation$coverage)))
